# filestore_fully_working.py - COMPLETE ERROR-FREE VERSION
import asyncio
import os
import sqlite3
import uuid
...
###############################################################################
def init_database():
    """Initialize SQLite database with proper SQL syntax"""
    try:
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS authorized_users (
                ...
                caption_disabled BOOLEAN DEFAULT 0
            )
        """)
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS file_links (
                ...
                is_active BOOLEAN DEFAULT 1,
                -- Foreign keys with ON DELETE CASCADE to ensure links are invalidated
                -- when the associated file or group is deleted.
        ...
        conn.execute("INSERT OR IGNORE INTO bot_settings (key, value) VALUES ('caption_enabled', '1')")
        # Corrected typo here: CUSTOM_CAPPTION -> CUSTOM_CAPTION
        conn.execute("INSERT OR IGNORE INTO bot_settings (key, value) VALUES ('custom_caption', ?)", (CUSTOM_CAPTION,))

        # Add admins to authorized users
        for admin_id in ADMIN_IDS:
            conn.execute("""
                INSERT OR IGNORE INTO authorized_users (user_id, username, first_name, added_by, is_active)
                VALUES (?, ?, ?, ?, 1)
...
def get_file_caption(file_name: str, serial_number: int = None, user_id: int = None) -> str:
    ...
    try:
        ...
        if user_id and not is_admin(user_id):
            conn = sqlite3.connect(DB_PATH)
            user_caption_disabled = conn.execute("""
                SELECT caption_disabled FROM authorized_users WHERE user_id = ?
            """, (user_id,)).fetchone()
            conn.close()
...
Contact Admin: {ADMIN_CONTACT} 👨‍💻"""
    ...
    async def add_user_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            existing = conn.execute("SELECT user_id FROM authorized_users WHERE user_id = ?", (user_id,)).fetchone()
    ...
    async def remove_user_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            conn = sqlite3.connect(DB_PATH)
            result = conn.execute("DELETE FROM authorized_users WHERE user_id = ?", (user_id,))
    ...
    async def list_users_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            users = conn.execute("""
                ...
                FROM authorized_users WHERE user_id NOT IN (?, ?)
                ORDER BY added_at DESC
            """, (ADMIN_IDS[0], ADMIN_IDS[1])).fetchall()
            conn.close()
    ...
    async def getlink_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            file_info = conn.execute("""
                ...
                LEFT JOIN file_links fl ON f.id = fl.file_id AND fl.link_type = 'file' AND fl.owner_id = ? AND fl.is_active = 1
                WHERE g.name = ? AND f.serial_number = ? AND g.owner_id = ?
            """, (user_id, group_name, file_serial_number, user_id)).fetchone()
    ...
    async def deletefile_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            file_info = conn.execute("""
                ...
                JOIN groups g ON f.group_id = g.id
                WHERE g.name = ? AND f.serial_number = ? AND g.owner_id = ?
            """, (group_name, file_serial_number, user_id)).fetchone()
    ...
    async def deletegroup_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            conn = sqlite3.connect(DB_PATH)
            group_info = conn.execute("SELECT id FROM groups WHERE name = ? AND owner_id = ?", (group_name, user_id)).fetchone()
    ...
    async def getgrouplink_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            group_info = conn.execute("""
                SELECT id FROM groups WHERE name = ? AND owner_id = ?
            """, (group_name, user_id)).fetchone()
            ...
            link_info = conn.execute("""
                ...
                WHERE group_id = ? AND owner_id = ? AND link_type = 'group' AND is_active = 1
    ...
    async def _execute_revoke_link(self, update: Update, link_code: str, user_id: int):
        ...
        try:
            ...
            # Invalidate the link
            conn.execute("""
                UPDATE file_links SET is_active = 0 WHERE id = ?
            """, (link_db_id,))
            conn.commit()
            conn.close()
...
Choose an option below to get started! 👇"""
    ...
    async def _handle_single_file(self, update: Update, context: ContextTypes.DEFAULT_TYPE, file_obj, file_type: str, file_name: str, file_size: int):
        ...
        try:
            ...
            try:
                ...
                conn = sqlite3.connect(DB_PATH)
                conn.execute("""
                    UPDATE files SET storage_message_id = ? WHERE id = ?
    ...
    async def _handle_bulk_file(self, update: Update, context: ContextTypes.DEFAULT_TYPE, file_obj, file_type: str, file_name: str, file_size: int):
        ...
        try:
            ...
            # Add file to bulk session
            self.bulk_sessions[user_id]['files'].append({
                'obj': file_obj,
                'type': file_type,
                'name': file_name,
                'size': file_size
            ...
            await update.message.reply_text(
                ...
                f"Continue adding files or finish upload.",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )

        except Exception as e:
            logger.error(f"Bulk file handler error: {e}")
            await update.message.reply_text("Error processing bulk file 😔")

    async def _finish_bulk_upload(self, query, context):
        """Finish bulk upload session with actual processing and rate limiting."""
        try:
            user_id = query.from_user.id
            ...
            if not files:
                await query.edit_message_text("No files in bulk session 🤷‍♂️")
                return

            processing_message = await query.edit_message_text(
                f"Processing {len(files)} files for group '{group_name}'... ⏳\n\n"
                "This might take a while due to Telegram's rate limits. Please wait."
            )
            ...
            for i, file_data in enumerate(files):
                try:
                    ...
                    # Ensure the message is still editable before attempting edit
                    try:
                        await processing_message.edit_text(
                            f"Processing file {i+1}/{len(files)}: {file_data['name']} ⏳\n\n"
                            "This might take a while due to Telegram's rate limits. Please wait."
                        )
                    except Exception:
                        # If message can't be edited (e.g., deleted by Telegram), send a new one
                    ...
                    # Save to database
                    file_id, serial_number = await self._save_file_to_db(
                        user_id, group_name, file_data['obj'], file_data['type'],
                        file_data['name'], file_data['size']
                    )

                    # Generate caption
                    caption = get_file_caption(file_data['name'], serial_number, user_id)

                    # Upload to storage channel
                    storage_msg = await self._send_to_storage(file_data['obj'], file_data['type'], caption)
                    ...
                    # Introduce delay to avoid flood limits
                    await asyncio.sleep(BULK_UPLOAD_DELAY)

                except Exception as e:
                    logger.error(f"Error uploading file {file_data['name']} during bulk upload: {e}")
                    # Inform user about skipped file
                    await query.message.reply_text(f"Skipped file '{file_data['name']}' due to an error: {e}. ❌")
                    continue
            ...
            if uploaded_files:
                ...
                group_id_row = conn.execute("""
                    SELECT id FROM groups WHERE owner_id = ? AND name = ?
                ...
                keyboard = [
                    [InlineKeyboardButton("Share Group 🔗", url=share_link)],
                    [InlineKeyboardButton("Bulk Upload Again 📦", callback_data="cmd_bulkupload")],
                    [InlineKeyboardButton("Main Menu 🏠", callback_data="main_menu")]
                ]
            ...
            logger.error(f"Bulk upload completion error: {e}")
            await query.message.reply_text(f"Error completing bulk upload: {e} 😔")

    async def _cancel_bulk_upload(self, query, context):
        ...
        try:
            ...
            if user_id in self.bulk_sessions:
                ...
                await query.edit_message_text(
                    ...
                    f"Session cancelled with {file_count} files.\n"
            ...
            logger.error(f"Cancel bulk upload error: {e}")
            await query.edit_message_text("Error cancelling bulk upload 😔")
    ...
    async def _save_file_to_db(self, user_id: int, group_name: str, file_obj, file_type: str, file_name: str, file_size: int) -> Tuple[int, int]:
        """Save file to database and return file_id, serial_number"""
        conn = sqlite3.connect(DB_PATH)

        # Get or create group
        group_row = conn.execute("""
            SELECT id, total_files FROM groups WHERE owner_id = ? AND name = ?
        """, (user_id, group_name)).fetchone()
        ...
        if group_row:
            ...
            # This case should ideally not happen if 'Add More Files' is used for existing groups
            # but is kept for robustness.
            group_id = conn.execute("""
                INSERT INTO groups (name, owner_id, total_files, total_size)
                VALUES (?, ?, 0, 0)
            """, (group_name, user_id)).lastrowid
            serial_number = 1
        ...
        """, (group_id, serial_number, unique_id, file_name, file_type,
              file_size, file_obj.file_id, user_id, file_obj.file_unique_id or "")).lastrowid

        # Update group stats
        conn.execute("""
            UPDATE groups SET total_files = ?, total_size = total_size + ?
            WHERE id = ?
    ...
    async def _send_to_storage(self, file_obj, file_type: str, caption: str):
    ...
    # ================= COMPLETE ADMIN INTERFACE METHODS =================

    async def _show_admin_panel(self, message):
        """Show admin panel with complete stats"""
        try:
            conn = sqlite3.connect(DB_PATH)

            total_users = conn.execute("""
                SELECT COUNT(*) FROM authorized_users
                WHERE is_active = 1 AND user_id NOT IN (?, ?)
            """, (ADMIN_IDS[0], ADMIN_IDS[1])).fetchone()[0]
            total_groups = conn.execute("SELECT COUNT(*) FROM groups").fetchone()[0]
            total_files = conn.execute("SELECT COUNT(*) FROM files").fetchone()[0]
            total_links = conn.execute("SELECT COUNT(*) FROM file_links WHERE is_active = 1").fetchone()[0]
            total_size = conn.execute("SELECT COALESCE(SUM(total_size), 0) FROM groups").fetchone()[0]

            conn.close()

            caption_enabled, custom_caption = get_caption_setting()
            caption_status = "Enabled ✅" if caption_enabled else "Disabled ❌"
...
Current Settings ⚙️:
- Global Caption: {caption_status}
- Caption Text: {custom_caption}
- File Size Limit: {format_size(MAX_FILE_SIZE)}
...
Contact: {ADMIN_CONTACT} 📞"""
            ...
            await message.reply_text("Error loading admin panel 😔")
...
- Text: {custom_caption[:30]}{'...' if len(custom_caption) > 30 else ''}
...
Choose an action: 👇"""
            ...
            keyboard = [
                [
                    ...
                    InlineKeyboardButton("Caption ✍️", callback_data="caption_settings")
                ],
                [
            ...
            logger.error(f"Admin panel callback error: {e}")
            await query.edit_message_text("Error loading admin panel 😔")

    # ================= FIXED BOT STATS METHODS =================

    async def _show_bot_stats_callback(self, query):
        """Show bot stats via callback - FIXED VERSION"""
        try:
            conn = sqlite3.connect(DB_PATH)

            total_users = conn.execute("""
                SELECT COUNT(*) FROM authorized_users WHERE user_id NOT IN (?, ?)
            """, (ADMIN_IDS[0], ADMIN_IDS[1])).fetchone()[0]
            active_users = conn.execute("""
                SELECT COUNT(*) FROM authorized_users WHERE is_active = 1 AND user_id NOT IN (?, ?)
            """, (ADMIN_IDS[0], ADMIN_IDS[1])).fetchone()[0]
            total_groups = conn.execute("SELECT COUNT(*) FROM groups").fetchone()[0]
            total_files = conn.execute("SELECT COUNT(*) FROM files").fetchone()[0]
            total_size = conn.execute("SELECT COALESCE(SUM(total_size), 0) FROM groups").fetchone()[0]
            active_links = conn.execute("SELECT COUNT(*) FROM file_links WHERE is_active = 1").fetchone()[0]

            conn.close()

            caption_enabled, _ = get_caption_setting()
...
File Limit: {format_size(MAX_FILE_SIZE)}"""
    ...
    async def _show_full_stats_callback(self, query):
        """Show full detailed statistics - WORKING VERSION"""
        try:
            conn = sqlite3.connect(DB_PATH)
            ...
            stats['active_users'] = conn.execute("""
                SELECT COUNT(*) FROM authorized_users WHERE is_active = 1 AND user_id NOT IN (?, ?)
            """, (ADMIN_IDS[0], ADMIN_IDS[1])).fetchone()[0]
            stats['users_no_caption'] = conn.execute("""
                SELECT COUNT(*) FROM authorized_users WHERE caption_disabled = 1 AND user_id NOT IN (?, ?)
            """, (ADMIN_IDS[0], ADMIN_IDS[1])).fetchone()[0]
            ...
            stats['total_size'] = conn.execute("SELECT COALESCE(SUM(total_size), 0) FROM groups").fetchone()[0]
            stats['active_links'] = conn.execute("SELECT COUNT(*) FROM file_links WHERE is_active = 1").fetchone()[0]
            stats['total_clicks'] = conn.execute("SELECT COALESCE(SUM(clicks), 0) FROM file_links").fetchone()[0]

            # Recent activity (last 24 hours)
            recent_uploads = conn.execute("""
                SELECT COUNT(*) FROM files WHERE uploaded_at > datetime('now', '-24 hours')
            """).fetchone()[0]

            recent_users = conn.execute("""
                SELECT COUNT(*) FROM authorized_users WHERE added_at > datetime('now', '-24 hours')
            """).fetchone()[0]
            ...
            text = f"""Detailed Bot Statistics 📈

Users:
- Total: {stats['total_users']} 👥
- Active: {stats['active_users']} ✅
- Without Caption: {stats['users_no_caption']} 🚫
...
- Avg per Group: {stats['total_files'] // max(stats['total_groups'], 1)}

Links & Activity:
- Active Links: {stats['active_links']} 🔗
- Total Clicks: {stats['total_clicks']} 🖱️
- Recent Uploads (24h): {recent_uploads} ⬆️
...
- Contact: {ADMIN_CONTACT} 📞"""
    ...
    async def _export_stats_callback(self, query):
        """Export statistics data - WORKING VERSION"""
        try:
            conn = sqlite3.connect(DB_PATH)

            # Generate export data
            export_data = f"""Bot Statistics Export 📤
Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

USERS:
Total: {conn.execute('SELECT COUNT(*) FROM authorized_users').fetchone()[0]}
Active: {conn.execute('SELECT COUNT(*) FROM authorized_users WHERE is_active=1').fetchone()[0]}
Admins: {len(ADMIN_IDS)}

CONTENT:
Groups: {conn.execute('SELECT COUNT(*) FROM groups').fetchone()[0]}
Files: {conn.execute('SELECT COUNT(*) FROM files').fetchone()[0]}
Total Size: {format_size(conn.execute('SELECT COALESCE(SUM(total_size), 0) FROM groups').fetchone()[0])}

LINKS:
Active: {conn.execute('SELECT COUNT(*) FROM file_links WHERE is_active=1').fetchone()[0]}
Total Clicks: {conn.execute('SELECT COALESCE(SUM(clicks), 0) FROM file_links').fetchone()[0]}

SETTINGS:
File Limit: {format_size(MAX_FILE_SIZE)}
Caption: {"On" if get_caption_setting()[0] else "Off"}
Admin Contact: {ADMIN_CONTACT}

RECENT ACTIVITY (24h):
New Uploads: {conn.execute("SELECT COUNT(*) FROM files WHERE uploaded_at > datetime('now', '-24 hours')").fetchone()[0]}
New Users: {conn.execute("SELECT COUNT(*) FROM authorized_users WHERE added_at > datetime('now', '-24 hours')").fetchone()[0]}"""
            ...
            export_file = io.BytesIO(export_data.encode('utf-8'))
            export_file.name = f"bot_stats_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            ...
            try:
                ...
                await query.edit_message_text(
                    "Statistics exported successfully! ✅\n\nCheck the document above for detailed data.",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("Bot Stats 📊", callback_data="bot_stats")]
                    ])
                )
                ...
                await query.edit_message_text(
                    f"Statistics Export:\n\n{export_data[:3000]}{'...' if len(export_data) > 3000 else ''} 😔",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("Bot Stats 📊", callback_data="bot_stats")]
                    ])
                )
            ...
            await query.edit_message_text("Error exporting statistics 😔")

    async def _show_usage_report_callback(self, query):
        """Show a basic usage report."""
        try:
            conn = sqlite3.connect(DB_PATH)

            # Get data for last 24 hours
            new_users_24h = conn.execute("SELECT COUNT(*) FROM authorized_users WHERE added_at > datetime('now', '-24 hours')").fetchone()[0]
            new_uploads_24h = conn.execute("SELECT COUNT(*) FROM files WHERE uploaded_at > datetime('now', '-24 hours')").fetchone()[0]
            clicks_24h = conn.execute("SELECT COALESCE(SUM(clicks), 0) FROM file_links WHERE created_at > datetime('now', '-24 hours')").fetchone()[0]

            # Get data for last 7 days
            new_users_7d = conn.execute("SELECT COUNT(*) FROM authorized_users WHERE added_at > datetime('now', '-7 days')").fetchone()[0]
            new_uploads_7d = conn.execute("SELECT COUNT(*) FROM files WHERE uploaded_at > datetime('now', '-7 days')").fetchone()[0]
            clicks_7d = conn.execute("SELECT COALESCE(SUM(clicks), 0) FROM file_links WHERE created_at > datetime('now', '-7 days')").fetchone()[0]
...
- New Uploads: {new_uploads_24h} ⬆️
- Link Clicks: {clicks_24h} 🖱️

Last 7 Days 🗓️:
- New Users: {new_users_7d} 👶
- New Uploads: {new_uploads_7d} ⬆️
- Link Clicks: {clicks_7d} 🖱️

This report provides a quick overview of recent bot activity. ✨"""

            keyboard = [[InlineKeyboardButton("Bot Stats 📊", callback_data="bot_stats")]]
            await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))

        except Exception as e:
            logger.error(f"Error generating usage report: {e}")
            await query.edit_message_text("Error generating usage report. 😔")

    async def _show_detailed_stats(self, message):
        """Show detailed stats with safe formatting"""
        try:
            conn = sqlite3.connect(DB_PATH)
...
- Contact: {ADMIN_CONTACT} 📞"""
    ...
    async def _show_my_links(self, query, user_id):
        ...
        try:
            ...
            links = conn.execute("""
                ...
                WHERE fl.owner_id = ? AND fl.is_active = 1
    ...
    async def _handle_link_access(self, update: Update, context: ContextTypes.DEFAULT_TYPE, link_code: str):
        ...
        try:
            ...
            # Update clicks
            conn.execute("""
                UPDATE file_links SET clicks = clicks + 1 WHERE link_code = ?
            """, (link_code,))
            conn.commit()
            conn.close()
    ...
    async def _toggle_global_caption(self, query):
        ...
        conn = sqlite3.connect(DB_PATH)
        conn.execute("UPDATE bot_settings SET value = ? WHERE key = 'caption_enabled'", ('1' if new_status else '0',))
        conn.commit()
        conn.close()
    ...
    async def _toggle_user_caption(self, query, data):
        ...
        conn = sqlite3.connect(DB_PATH)
        current = conn.execute("SELECT caption_disabled, first_name FROM authorized_users WHERE user_id = ?", (user_id,)).fetchone()

        if current:
            new_status = not current[0]
            conn.execute("UPDATE authorized_users SET caption_disabled = ? WHERE user_id = ?", (new_status, user_id))
            conn.commit()
...
No configurable options here yet.
    ...
    async def _update_custom_caption(self, update: Update, new_caption: str):
        ...
        if user_id in self.caption_edit_pending and self.caption_edit_pending[user_id]['state'] == 'waiting_for_caption':
            try:
                conn = sqlite3.connect(DB_PATH)
                conn.execute("UPDATE bot_settings SET value = ? WHERE key = 'custom_caption'", (new_caption,))
                conn.commit()
                conn.close()
    ...
    async def _show_user_info(self, query, data):
        ...
        try:
            ...
            if user_info:
                ...
                if added_by:
                    conn = sqlite3.connect(DB_PATH)
                    admin_name_row = conn.execute("SELECT first_name FROM authorized_users WHERE user_id = ?", (added_by,)).fetchone()
                    conn.close()
                    if admin_name_row:
...
Added At: {added_at[:16]}""" # Slice for cleaner timestamp
                ...
                keyboard = [
                    [InlineKeyboardButton("Toggle Caption ✍️", callback_data=f"toggle_user_caption_{user_id}")],
                    # FIX: Removed trailing ')' from callback_data
    ...
    async def _confirm_user_removal(self, query, data):
        ...
        conn = sqlite3.connect(DB_PATH)
        user_name = conn.execute("SELECT first_name FROM authorized_users WHERE user_id = ?", (user_id_to_remove,)).fetchone()
        conn.close()
    ...
    async def _execute_user_removal(self, query, data):
        ...
        try:
            conn = sqlite3.connect(DB_PATH)
            result = conn.execute("DELETE FROM authorized_users WHERE user_id = ?", (user_id_to_remove,))
            conn.commit()
            conn.close()
    ...
    async def _handle_view_group(self, query, data):
        ...
        try:
            ...
            group_info = conn.execute("""
                ...
                FROM groups WHERE id = ? AND owner_id = ?
            """, (group_id, user_id)).fetchone()
...
Files in this group (first 10):"""
    ...
    async def _list_group_files(self, query, data):
        ...
        try:
            conn = sqlite3.connect(DB_PATH)
            group_info = conn.execute("SELECT name FROM groups WHERE id = ? AND owner_id = ?", (group_id, user_id)).fetchone()
    ...
    async def _view_file_details(self, query, data):
        ...
        try:
            ...
            file_link_row = conn.execute("""
                SELECT link_code FROM file_links WHERE file_id = ? AND link_type = 'file' AND owner_id = ? AND is_active = 1
            """, (file_id, user_id)).fetchone()
...
File Link: {file_link_text}"""
    ...
    async def _confirm_delete_file(self, query, data):
        ...
        try:
            ...
            file_info = conn.execute("""
                ...
                JOIN groups g ON f.group_id = g.id
                WHERE f.id = ? AND g.owner_id = ?
            """, (file_id_to_delete, user_id)).fetchone()
            conn.close()
    ...
    async def _execute_delete_file(self, query, data):
        ...
        try:
            ...
            # Delete file record
            result = conn.execute("DELETE FROM files WHERE id = ?", (file_id_to_delete,))

            if result.rowcount > 0:
                # Update group statistics
                conn.execute("""
                    UPDATE groups SET total_files = total_files - 1, total_size = total_size - ?
                    WHERE id = ?
                """, (file_size, group_id))
                ...
                # conn.execute("UPDATE file_links SET is_active = 0 WHERE file_id = ?", (file_id_to_delete,))
    ...
    async def _confirm_delete_group(self, query, data):
        ...
        try:
            conn = sqlite3.connect(DB_PATH)
            group_name_row = conn.execute("SELECT name FROM groups WHERE id = ? AND owner_id = ?", (group_id_to_delete, user_id)).fetchone()
            conn.close()
    ...
    async def _execute_delete_group(self, query, data):
        ...
        try:
            ...
            result = conn.execute("DELETE FROM groups WHERE id = ?", (group_id_to_delete,))
